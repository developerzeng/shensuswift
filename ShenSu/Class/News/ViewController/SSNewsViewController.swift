...
import EasyPeasy
import SwiftyJSON
class SSNewsViewController: BaseViewController {
	var tableView: UITableView!
    var titleName:String!
	var newsData = Array<SSNewsModel>()
    var didSelectedRowBlock:((SSNewsModel)->())?
...
extension SSNewsViewController: UITableViewDelegate, UITableViewDataSource {
	...
	func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
		var cell = tableView.dequeueReusableCell(withIdentifier: "cell") as? SSNewsTableViewCell
		if cell == nil {
			cell = Bundle.main.loadNibNamed("SSNewsTableViewCell", owner: self, options: nil)?.first as? SSNewsTableViewCell
		}

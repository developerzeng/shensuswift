...
- (void)initUI{
    ...
    [self setNavTitleWithTitle:@"社区" color:nil];
    ...
    [self.tableView addRefreshingHeaderViewWithRefreshingBlock:^{
        [weak requestDataLoadNew:YES];
    }];
    [self.tableView addRefreshingFooterViewWithRefreshingBlock:^{
        [weak requestDataLoadNew:NO];
    }];
...
- (void)requestDataLoadNew:(BOOL)loadNew{
    ...
    dic[@"type"] = @(_curPageIndex);
    dic[@"isbreak"] = @"0";
    dic[@"pn"] = @(_currentRequestPage);
    [[ObjectNet default] requesMethodtWithUrlWithUrl:@"http://ews.500.com/sns/app/newfilter?"  parameters:dic  completionHandler:^( NetworkStatus status , id data) {
        [self.tableView endRefreshing];
        NSDictionary * dic = [NSDictionary dictionaryWithDictionary:data];
        if (status == NetworkStatusSuccess && [dic[@"status"] intValue] == 100){
            [dic[@"data"][@"list"] enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                YHWorkGroup * model = [[YHWorkGroup alloc] init];
                YHUserInfo * user = [[YHUserInfo alloc] init];
                user.userName = obj[@"username"];
                user.oriAvaterUrl = [NSURL URLWithString:obj[@"photo"][@"fullsize"]];
                user.avatarUrl = [NSURL URLWithString:obj[@"photo"][@"thumbnail"]];
                user.company = [NSString stringWithFormat:@"%@%@",obj[@"location"][@"city"],obj[@"location"][@"address"]];
                model.userInfo = user;
                model.dynamicId = obj[@"_id"];
                model.type = 0;
                model.publishTime = obj[@"date"];
                model.msgContent = obj[@"content"][@"title"];
                model.commentCount = arc4random()%100;
                model.likeCount = arc4random()%50;
                model.isLike = false;
                NSArray * array = [NSArray arrayWithArray:obj[@"content"][@"images"]];
                NSMutableArray * originalPicUrls = [[NSMutableArray alloc] init];
                NSMutableArray * thumbnailPicUrls = [[NSMutableArray alloc] init];
                [array enumerateObjectsUsingBlock:^(id  _Nonnull cobj, NSUInteger idx, BOOL * _Nonnull stop) {
                    [originalPicUrls addObject:[NSURL URLWithString:cobj[@"fullsize"]]];
                    [thumbnailPicUrls addObject:[NSURL URLWithString:cobj[@"thumbnail"]]];
                }];
                model.originalPicUrls = originalPicUrls;
                model.thumbnailPicUrls = thumbnailPicUrls;
...
- (void)onCommentInCell:(CellForWorkGroup *)cell{
    YHTimeDetaTableView * vc = [[YHTimeDetaTableView alloc] init];
    vc.dataModel = cell.model;

...
class SSMyViewController: BaseViewController {
	var collectionView: UICollectionView!
	var spacefloat: CGFloat = 5.0
	var imageHeight = UIScreen.main.bounds.width * 1200 / 1920
	var userHeader: SSUserHeaderCollectionReusableView?
	...
	override func viewDidLoad() {
		...
		collectionView.register(UINib.init(nibName: "SSMycenterCollectionViewCell", bundle: Bundle.main), forCellWithReuseIdentifier: "cell")
		collectionView.register(UINib.init(nibName: "SSUserHeaderCollectionReusableView", bundle: Bundle.main), forSupplementaryViewOfKind: UICollectionElementKindSectionHeader, withReuseIdentifier: "header")
...
extension SSMyViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
	...
	func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
		let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "cell", for: indexPath) as? SSMycenterCollectionViewCell
		cell?.setModel(model: dataArray[indexPath.row])
		return cell!
	}
	func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
		if kind == UICollectionElementKindSectionHeader {
			userHeader = collectionView.dequeueReusableSupplementaryView(ofKind: UICollectionElementKindSectionHeader, withReuseIdentifier: "header", for: indexPath) as? SSUserHeaderCollectionReusableView
	...
	func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
		...
		case 4:
			let vc = SSAboutUsViewController()
			_ = self.navigationController?.pushViewController(vc, animated: true)
			break
		default:
			break

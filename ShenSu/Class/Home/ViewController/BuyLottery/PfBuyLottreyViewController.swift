//
//  PfBuyLottreyViewController.swift
//  ShenSu
//
...
class PfBuyLottreyViewController: BaseViewController {
	var lotteryData: Array<ruleModel>? {
		didSet {
			lotteryData?.enumerated().forEach({ (index, rule) in
				var classArray = Array<PfBuyPfLotteryModel>()
				let min = Int(rule.Nmin) ?? 0
				let max = Int(rule.Nmax) ?? 0
				for i in min...max {
					let model = PfBuyPfLotteryModel()
					model.number = i.toString
					model.isChoose = false
					classArray.append(model)
				}
				selectedArray.append(Array<PfBuyPfLotteryModel>())
				dataArray.append(classArray)
			})

		}
	}
    var lotterydeta = PfLotteryDetaView()
	var qishu: String = ""
	var lotteryinfoArray = Array<PfLotteryModel>()
	var url: String!
    ...
    var lotteryInfoModel: PfHomePfLotteryModel!{
        didSet{
	...
	var buyListModels = Array<PfLotteryListModel>()
	var buyNumber: String = ""
	var buyViewHeaderView: PfBuyViewHeaderView!
	var footerView: PfBuyLotteryFooterView!
	var selectedArray = Array<Array<PfBuyPfLotteryModel>>()
	var collevtionView: UICollectionView!
	var insetspace: CGFloat = 10.0
	var dataArray = Array<Array<PfBuyPfLotteryModel>>()
	override func viewDidLoad() {
		super.viewDidLoad()
		addPfBuyViewHeaderView()
		addCollectionView()
		setCPData()
		...
		self.rightButtonClicked = { [weak self] btn in
            ...
            self?.lotterydeta.selectRowBlock = {index in
                ...
                switch index.row {
                case 0:
                    let vc = UserPfBuyListViewController()
                    vc.lotteryType = .lotterSaveTypeBuy
                    _ = self?.navigationController?.pushViewController(vc, animated: true)
                case 1:
                    let vc = PfLotteryRuleViewController()
                    vc.talk = self?.lotteryInfoModel.talk
                    _ = self?.navigationController?.pushViewController(vc, animated: true)
                default:
                    let model = PfLotteryJSModel()
                    model.caipiaoid = (self?.caipiaoid)!
                    let vc = PfPfLotteryDetaViewController()
...
//			self?.moveiphone()
	...
	func addPfBuyViewHeaderView() {
		buyViewHeaderView = PfBuyViewHeaderView()
	...
	func addCollectionView() {
		...
		collevtionView.register(PfBuyLottreyCollectionViewCell.self, forCellWithReuseIdentifier: "cell")
		collevtionView.register(PfBuyLotteryHeardCollectionReusableView.classForCoder(), forSupplementaryViewOfKind: UICollectionElementKindSectionHeader, withReuseIdentifier: "Header")
		self.view.addSubview(collevtionView)
		collevtionView <- [
			Edges(UIEdgeInsets(top: 30, left: 0, bottom: 50, right: 0))
		]

		footerView = PfBuyLotteryFooterView()
		...
		footerView.goBtnBlock = { [weak self] in
			...
			if self?.isChoose == true && self?.qishu.isEmpty == false {
				...
				let vc = PfBuyListViewController()
...
extension PfBuyLottreyViewController: UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout, BuyListreloadData {
	func reloadDataModels(models: Array<PfLotteryListModel>) {
	...
	func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
		let cell = collevtionView.dequeueReusableCell(withReuseIdentifier: "cell", for: indexPath) as? PfBuyLottreyCollectionViewCell
	...
	func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
		let view = collectionView.dequeueReusableSupplementaryView(ofKind: UICollectionElementKindSectionHeader, withReuseIdentifier: "Header", for: indexPath) as? PfBuyLotteryHeardCollectionReusableView

//
//  SSBuyLottreyViewController.swift
//  ShenSu
//
...
class SSBuyLottreyViewController: BaseViewController {
	var lotteryData: Array<ruleModel>? {
		didSet {
			lotteryData?.enumerated().forEach({ (index, rule) in
				var classArray = Array<SSBuySSLotteryModel>()
				let min = Int(rule.Nmin) ?? 0
				let max = Int(rule.Nmax) ?? 0
				for i in min...max {
					let model = SSBuySSLotteryModel()
					model.number = i.toString
					model.isChoose = false
					classArray.append(model)
				}
				selectedArray.append(Array<SSBuySSLotteryModel>())
				dataArray.append(classArray)
			})

		}
	}
    var lotterydeta = SSLotteryDetaView()
	var qishu: String = ""
	var lotteryinfoArray = Array<SSLotteryModel>()
	var url: String!
    ...
    var lotteryInfoModel: SSHomeSSLotteryModel!{
        didSet{
	...
	var buyListModels = Array<SSLotteryListModel>()
	var buyNumber: String = ""
	var buyViewHeaderView: SSBuyViewHeaderView!
	var footerView: SSBuyLotteryFooterView!
	var selectedArray = Array<Array<SSBuySSLotteryModel>>()
	var collevtionView: UICollectionView!
	var insetspace: CGFloat = 10.0
	var dataArray = Array<Array<SSBuySSLotteryModel>>()
	override func viewDidLoad() {
		super.viewDidLoad()
		addSSBuyViewHeaderView()
		addCollectionView()
		setCPData()
		...
		self.rightButtonClicked = { [weak self] btn in
            ...
            self?.lotterydeta.selectRowBlock = {index in
                ...
                switch index.row {
                case 0:
                    let vc = UserSSBuyListViewController()
                    vc.lotteryType = .lotterSaveTypeBuy
                    _ = self?.navigationController?.pushViewController(vc, animated: true)
                case 1:
                    let vc = SSLotteryRuleViewController()
...
//			self?.moveiphone()
	...
	func addSSBuyViewHeaderView() {
		buyViewHeaderView = SSBuyViewHeaderView()
	...
	func addCollectionView() {
		...
		collevtionView.register(SSBuyLottreyCollectionViewCell.self, forCellWithReuseIdentifier: "cell")
		collevtionView.register(SSBuyLotteryHeardCollectionReusableView.classForCoder(), forSupplementaryViewOfKind: UICollectionElementKindSectionHeader, withReuseIdentifier: "Header")
		...
		footerView.goBtnBlock = { [weak self] in
			...
			if self?.isChoose == true && self?.qishu.isEmpty == false {
				...
				let vc = SSBuyListViewController()
...
extension SSBuyLottreyViewController: UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout, BuyListreloadData {
	...
	func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
		let cell = collevtionView.dequeueReusableCell(withReuseIdentifier: "cell", for: indexPath) as? SSBuyLottreyCollectionViewCell
	...
	func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
		let view = collectionView.dequeueReusableSupplementaryView(ofKind: UICollectionElementKindSectionHeader, withReuseIdentifier: "Header", for: indexPath) as? SSBuyLotteryHeardCollectionReusableView

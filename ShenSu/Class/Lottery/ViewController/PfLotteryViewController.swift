...
class PfLotteryViewController: BaseViewController, UITableViewDelegate, UITableViewDataSource {
	var tableView: UITableView!
	var lotteryArray = Array<PfHomePfLotteryModel>()
	var lotteryinfoArray = Array<PfLotteryJSModel>()
    var cpType:CpType = .allCp
    var gaopingArray:Array<PfLotteryJSModel> {
       let array = ["73","82","93","90","75","69","83","115","94"]
       var gpA = Array<PfLotteryJSModel>()
    ...
    var dipingArray:Array<PfLotteryJSModel>{
        let array = ["11","14","12"]
        var gpA = Array<PfLotteryJSModel>()
        lotteryinfoArray.enumerated().forEach { (index,model) in
            array.enumerated().forEach({ (_,cpid) in
	...
	func requestopenCode(model: PfHomePfLotteryModel, group: DispatchGroup) {
	...
	func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
		var cell = tableView.dequeueReusableCell(withIdentifier: "cell") as? PfLotteryTableViewCell
		if cell == nil {
			cell = Bundle.main.loadNibNamed("PfLotteryTableViewCell", owner: self, options: nil)?.first as? PfLotteryTableViewCell
		}

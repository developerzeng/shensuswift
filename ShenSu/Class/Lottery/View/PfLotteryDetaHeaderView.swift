...
import UIKit
import EasyPeasy
class PfLotteryDetaHeaderView: BaseView {
	...
	override init(frame: CGRect) {
		...
		openCodeView.addSubview(lotterPfNumberView)
		lotterPfNumberView <- [
			Edges(UIEdgeInsets(top: 0, left: 0, bottom: 0, right: 0))
		]
	}
	func setModel(model: PfLotteryJSModel, lotteryTypeName: String) {
